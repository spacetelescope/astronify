...
class SoniSeries():
    ...
    def sonification_method(self, value):
        assert value.lower() in ("pyo","thinkdsp"), 'The only valid sonification methods are pyo and thinkdsp.'
    ...
    def sonify(self):
        """
        TODO: document, also ad arguments
        """
        ...
        data["onsets"] = [x for x in (data["time"].jd - data["time"].jd[0])/exptime*spacing]
    ...
    def _pyo_play(self):
        ...
        # TODO: This doesn't seem like the best way to do this, but I don't know how to make it better
        env = pyo.Linseg(list=[(0,0),(.01,1),(duration-.1,1),(duration-.05,0.5),(duration-.005,0)],
                         mul=[.1 for i in range(len(pitches))]).play(delay=list(delays), dur=duration)

        self.streams = pyo.Sine(list(pitches), 0, env).out(delay=list(delays), dur=duration)
    ...
    def _pyo_write(self, filepath):
        ...
        env = pyo.Linseg(list=[(0,0),(.1,1),(duration-.1,1),(duration-.05,0.5),(duration-.005,0)],
                         mul=[.05 for i in range(len(pitches))]).play(delay=list(delays), dur=duration)
        sine = pyo.Sine(list(pitches), 0, env).out(delay=list(delays), dur=duration)
        self.server.start()
